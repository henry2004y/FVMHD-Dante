      ...
      function state = set_cell_boundary(obj,state)
         ...
         switch obj.TypeBC
            ...
            case 'float'
               ...
               state_GV(iMin:iMax,1:nG,kMin:kMax,:) = ...
                  repmat(state_GV(iMin:iMax,jMin,kMin:kMax,:),[1 nG 1 1]);
               state_GV(iMin:iMax,jMin:jMax,1:nG,:) = ...
                  repmat(state_GV(iMin:iMax,jMin:jMax,kMin,:),[1 1 nG 1]);
               state_GV(iMax+1:iMaxAll,jMin:jMax,kMin:kMax,:) = ...
                  repmat(state_GV(iMax,jMin:jMax,kMin:kMax,:),[nG 1 1 1]);
               state_GV(iMin:iMax,jMax+1:jMaxAll,kMin:kMax,:) = ...
                  repmat(state_GV(iMin:iMax,jMax,kMin:kMax,:),[1 nG 1 1]);
               state_GV(iMin:iMax,jMin:jMax,kMax+1:kMaxAll,:) = ...
                  repmat(state_GV(iMin:iMax,jMin:jMax,kMax,:),[1 1 nG 1]);
            otherwise
